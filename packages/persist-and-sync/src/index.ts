...
export const persistNSync: PersistNSyncType = (f, options) => (set, get, store) => {
	...
	const set_: typeof set = (p, replace) => {
		...
		if (savedState && initRef.init > 0) {
			set({ ...newState, ...JSON.parse(savedState) }, replace);
			initRef.init--;
		} else {
			// @ts-ignore
			set(newState, replace);
			saveAndSync({ newState, prevState, channel, regExpToIgnore, name });
		}
	};
...
function saveAndSync({ regExpToIgnore, newState, prevState, channel, name }: SaveAndSyncProps) {
	if (regExpToIgnore) {
